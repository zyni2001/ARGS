...
def graph_density(adj_matrix):
    adj_matrix = (adj_matrix.t() + adj_matrix)/2
    num_nodes = adj_matrix.shape[0]
    num_edges = torch.sum(adj_matrix).item() / 2  # each edge is counted twice in the adjacency matrix
    density = num_edges / (num_nodes * (num_nodes - 1) / 2)
    return density

# calculate a graph degree distribution mean
