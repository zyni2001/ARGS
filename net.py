...
class net_gcn(nn.Module):
    ...
    def forward(self, x, adj, val_test=False, dw_feature_pert=None):
        features = x
        ...
        ### attribute feat loss
        if dw_feature_pert is None:
            loss_smooth_feat = self.feature_smoothing(adj, features)
        else:
            loss_smooth_feat = 0

        # ### struct feat loss
        if dw_feature_pert is not None:
            loss_feat_struct = self.feature_smoothing(adj, dw_feature_pert)
        else:
            loss_feat_struct = 0

        return x, loss_smooth_feat + loss_feat_struct
    ...
    def cos_loss(self, S, X):
        X_normalized = X / X.norm(dim=1)[:, None]  # normalize the feature vectors

        # calculate the similarity matrix
        similarity_matrix = -torch.mm(X_normalized, X_normalized.t())
        
        # element-wise multiplication with the adjacency matrix
        loss_matrix = S * similarity_matrix
