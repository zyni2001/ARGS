...
def get_masked_values(mask_weight_tensor, threshold):
    zeros = torch.zeros_like(mask_weight_tensor)
    masked_values = torch.where(mask_weight_tensor.abs() > threshold, mask_weight_tensor, zeros)
...
def get_final_mask_epoch(model, adj_percent, wei_percent):
    ...
    mask_dict['mask_value'] = get_masked_values(ori_adj_mask, adj_thre)
